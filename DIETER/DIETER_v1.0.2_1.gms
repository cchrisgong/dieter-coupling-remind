...
P_RES("Solar") = sum(grade, remind_cap(yr, "DEU", "spv", grade))$sameas(yr,"2030") * 1e6;
P_RES("Wind_on") = sum(grade, remind_cap(yr, "DEU", "wind", grade))$sameas(yr,"2030") * 1e6;
*converting TWa to MWh
DIETER_OLDtotdem = sum( h , d_y_reg('2013',"IND",h));
d(h) = d_y_reg('2013',"IND", h) * remind_totdemand(yr, "DEU", "seel")$sameas(yr,"2030") * 365 * 24 * 1e6 / DIETER_OLDtotdem;
...
report(yr, varname, sto)$sameas(yr,"2030") = N_STO_P.l(sto);
