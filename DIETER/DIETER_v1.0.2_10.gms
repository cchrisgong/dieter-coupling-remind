...
<<<<<<< HEAD
earlyRetiCap_reporting(yr, reg, te_remind)$(remind_capEarlyReti(yr, reg, te_remind) ne 1) = (remind_capEarlyReti(yr, reg, te_remind) - remind_capEarlyReti2("2050", reg, te_remind) ) * remind_cap(yr, reg, te_remind, "1")
...
remind_VRECapFac("wind_on") = sum(grade, remind_pm_dataren("DEU", "nur", grade, "wind") * remind_vm_CapDistr("2055", "DEU", "wind", grade) / remind_cap("2055", "DEU", "wind", "1"));
remind_VRECapFac("Solar") = sum(grade, remind_pm_dataren("DEU", "nur", grade, "spv") * remind_vm_CapDistr("2055", "DEU", "spv", grade) / remind_cap("2055", "DEU", "spv", "1"));
remind_HydroCapFac = sum(grade, remind_pm_dataren("DEU", "nur", grade, "hydro") * remind_vm_CapDistr("2055", "DEU", "hydro", grade) / remind_cap("2055", "DEU", "hydro", "1"));
...
*
=======
P_RES.lo("Solar") = preInv_remind_prodSe("2055", "DEU", "pesol", "seel", "spv") * sm_TWa_2_MWh / ( remind_VRECapFac("Solar") * card(h)) ;
P_RES.lo("Wind_on") = preInv_remind_prodSe("2055", "DEU", "pewin", "seel", "wind") * sm_TWa_2_MWh / (remind_VRECapFac("Wind_on") * card(h)) ;
N_CON.lo("ror") = preInv_remind_prodSe("2055", "DEU", "pehyd", "seel", "hydro") * sm_TWa_2_MWh / (capfac_ror * 8760) ;
...
*                    ) * 1e6, 0.2* SMax(h, d(h)));
*
*N_CON.lo("OCGT_eff") = min(sum(grade, preInv_remind_cap("2055", "DEU", "ngt", grade)) * 1e6, 0.2* SMax(h, d(h)));
*
*      
...
*N_CON.lo("lig")$(sum(te_remind, earlyRetiCap_reporting("2055", "DEU", te_remind)$(COALte(te_remind))) = 0) = sum(te_remind,
*                    sum(   grade, preInv_remind_cap("2055", "DEU", te_remind, grade)$(COALte(te_remind))   )
*                    ) * 1e6 /2;
*
*N_CON.lo("hc")$(sum(te_remind, earlyRetiCap_reporting("2055", "DEU", te_remind)$(COALte(te_remind))) = 0) = sum(te_remind,
*                    sum(   grade, preInv_remind_cap("2055", "DEU", te_remind, grade)$(COALte(te_remind))   )
*                    ) * 1e6 /2;
*                    
*N_CON.lo("nuc")$(sum(te_remind, earlyRetiCap_reporting("2055", "DEU", te_remind)$(NUCte(te_remind))) = 0)  = sum(te_remind,
*                   sum(   grade, preInv_remind_cap("2055", "DEU", te_remind, grade)$(NUCte(te_remind))   )
*                  ) * 1e6;
*
*N_CON.lo("CCGT")$(sum(te_remind, earlyRetiCap_reporting("2055", "DEU", te_remind)$(NonPeakGASte(te_remind))) = 0) = sum(te_remind,
*                    sum(   grade, preInv_remind_cap("2055", "DEU", te_remind, grade)$(NonPeakGASte(te_remind))   )
*                    ) * 1e6;
*
*N_CON.lo("OCGT_eff")$(earlyRetiCap_reporting("2055", "DEU", "ngt") = 0) = sum(grade, preInv_remind_cap("2055", "DEU", "ngt", grade)) * 1e6;
*
*      
*N_CON.lo("bio")$(sum(te_remind, earlyRetiCap_reporting("2055", "DEU", te_remind)$(BIOte(te_remind))) = 0) =  sum(te_remind,
*                    sum(   grade, preInv_remind_cap("2055", "DEU", te_remind, grade)$(BIOte(te_remind))   )
*                    ) * 1e6;
*
**** if there is early retirement from last REMIND iteration, DIETER gets REMIND capacity as fx
*N_CON.fx("lig")$(sum(te_remind, earlyRetiCap_reporting("2055", "DEU", te_remind)$(COALte(te_remind))) > 0) =
*                        RM_postInv_cap_con("2055", "DEU", "coal") * 1e6 /2;
*
*N_CON.fx("hc")$(sum(te_remind, earlyRetiCap_reporting("2055", "DEU", te_remind)$(COALte(te_remind))) > 0) =
*                        RM_postInv_cap_con("2055", "DEU", "coal") * 1e6 /2;
*                    
*N_CON.fx("nuc")$(sum(te_remind, earlyRetiCap_reporting("2055", "DEU", te_remind)$(NUCte(te_remind))) > 0)  =
*                        RM_postInv_cap_con("2055", "DEU", "nuc") * 1e6;
*
*N_CON.fx("CCGT")$(sum(te_remind, earlyRetiCap_reporting("2055", "DEU", te_remind)$(NonPeakGASte(te_remind))) > 0) =
*                        RM_postInv_cap_con("2055", "DEU", "CCGT") * 1e6;
*
*N_CON.fx("OCGT_eff")$(earlyRetiCap_reporting("2055", "DEU", "ngt") > 0) = RM_postInv_cap_con("2055", "DEU", "OCGT_eff") * 1e6;
*
*
*N_CON.fx("bio")$(sum(te_remind, earlyRetiCap_reporting("2055", "DEU", te_remind)$(BIOte(te_remind))) > 0) =
*                        RM_postInv_cap_con("2055", "DEU", "bio") * 1e6;
...
con_fuelprice_reg0("lig",reg) = -remind_fuelcost2("2050",reg,"pecoal") / (-remind_budget2("2050",reg)) * 1e12 / sm_TWa_2_MWh * 1.2 - 3.6;
con_fuelprice_reg0("hc",reg) = -remind_fuelcost2("2050",reg,"pecoal") / (-remind_budget2("2050",reg)) * 1e12 / sm_TWa_2_MWh * 1.2 + 1.8;
con_fuelprice_reg0("CCGT",reg) = -remind_fuelcost2("2050",reg,"pegas") / (-remind_budget2("2050",reg)) * 1e12 / sm_TWa_2_MWh * 1.2;
con_fuelprice_reg0("bio",reg) = -remind_fuelcost2("2050",reg,"pebiolc") / (-remind_budget2("2050",reg)) * 1e12 / sm_TWa_2_MWh * 1.2;
con_fuelprice_reg0("nuc",reg) = -remind_fuelcost2("2050",reg,"peur") / (-remind_budget2("2050",reg)) * 1e12 / sm_TWa_2_MWh * 1.2;

con_fuelprice_reg1("lig",reg) = -remind_fuelcost("2055",reg,"pecoal") / (-remind_budget("2055",reg)) * 1e12 / sm_TWa_2_MWh * 1.2 - 3.6;
con_fuelprice_reg1("hc",reg) = -remind_fuelcost("2055",reg,"pecoal") / (-remind_budget("2055",reg)) * 1e12 / sm_TWa_2_MWh * 1.2 + 1.8;
con_fuelprice_reg1("CCGT",reg) = -remind_fuelcost("2055",reg,"pegas") / (-remind_budget("2055",reg)) * 1e12 / sm_TWa_2_MWh * 1.2;
con_fuelprice_reg1("bio",reg) = -remind_fuelcost("2055",reg,"pebiolc") / (-remind_budget("2055",reg)) * 1e12 / sm_TWa_2_MWh * 1.2;
con_fuelprice_reg1("nuc",reg) = -remind_fuelcost("2055",reg,"peur") / (-remind_budget("2055",reg)) * 1e12 / sm_TWa_2_MWh * 1.2;
...
execute_unload "results_DIETER_y10", report4RM, reportmk_4RM;
