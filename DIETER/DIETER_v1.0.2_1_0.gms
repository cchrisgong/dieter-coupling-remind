...
te_remind   remind technonlogy					  /spv, wind, elh2, ngcc, ngccc, ngt, bioigcc, pc,pcc, storspv, storwind/
pe_remind   remind primary energy                  /pegas, pecoal/
se_remind   remind secondary energy                /seel,seh2/
*omf is for fixed O&M cost
char_remind remind character                       /omf/
...
*================================================================
*=======read in capacity and capacity factor from remind ========
*converting TWa to MWh
P_RES("Solar") = sum(grade, remind_cap("2030", "DEU", "spv", grade)*remind_capFac("2030", "DEU", "spv")) * 1e6;
P_RES("Wind_on") = sum(grade, remind_cap("2030", "DEU", "wind", grade)*remind_capFac("2030", "DEU", "wind")) * 1e6;
P_RES("Wind_off") = 0;
*------------------------------------------------------------------
N_CON("nuc") = 0;
N_CON("ror") = 0;
...
N_CON("hc") = sum(grade, remind_cap("2030", "DEU", "pc", grade)*remind_capFac("2030", "DEU", "pc")) * 1e6
+sum(grade, remind_cap("2030", "DEU", "pcc", grade)*remind_capFac("2030", "DEU", "pcc")) * 1e6;

N_CON("CCGT") = sum(grade, remind_cap("2030", "DEU", "ngcc", grade)*remind_capFac("2030", "DEU", "ngcc")) * 1e6
                + sum(grade, remind_cap("2030", "DEU", "ngccc", grade)*remind_capFac("2030", "DEU", "ngccc")) * 1e6;

N_CON("OCGT_eff") = sum(grade, remind_cap("2030", "DEU", "ngt", grade)*remind_capFac("2030", "DEU", "ngt")) * 1e6;

N_CON("bio") = sum(grade, remind_cap("2030", "DEU", "bioigcc", grade)*remind_capFac("2030", "DEU", "bioigcc")) * 1e6;
*------------------------------------------------------------------
N_STO_P.fx('Sto1') = remind_cap("2030", "DEU", "storspv", "1") * 3 * 1e6+ remind_cap("2030", "DEU", "storwind", "1") * 0.3* 1e6;
*N_STO_P.fx('Sto7') = remind_cap("2030", "DEU", "elh2", "1") * 1e6;


*================================================================
*================ read in fuel price from remind ================
*1.2 is the conversion btw 2005$ and 2015$
*1e12 is the conversion btw Billion/kWh to $/MWh
*remind_budget is kind of like inflation rate
con_fuelprice_reg("hc",reg) = -remind_fuelcost("2030",reg,"pecoal")/ remind_budget("2030",reg) * 1e12 / sm_TWa_2_MWh * 1.2;
con_fuelprice_reg("CCGT",reg) = -remind_fuelcost("2030",reg,"pegas")/ remind_budget("2030",reg) * 1e12 / sm_TWa_2_MWh * 1.2;
con_fuelprice_reg("OCGT_ineff",reg) = con_fuelprice_reg("CCGT",reg);
...
c_i("hc") = remind_CapCost("2030", "DEU", "pc")/ remind_budget("2030","DEU") * 1e12/ sm_TWa_2_MWh * 1.2;
c_i("CCGT") = remind_CapCost("2030", "DEU", "ngcc") / remind_budget("2030","DEU")* 1e12/ sm_TWa_2_MWh * 1.2;
c_i("OCGT_eff") = remind_CapCost("2030", "DEU", "ngt") / remind_budget("2030","DEU")* 1e12/ sm_TWa_2_MWh * 1.2;
c_i("bio") = remind_CapCost("2030", "DEU", "bioigcc") / remind_budget("2030","DEU")* 1e12/ sm_TWa_2_MWh * 1.2;
 
c_i_res("Solar") = remind_CapCost("2030", "DEU", "spv") / remind_budget("2030","DEU")* 1e12/ sm_TWa_2_MWh * 1.2;
c_i_res("Wind_on") = remind_CapCost("2030", "DEU", "wind") / remind_budget("2030","DEU")* 1e12/ sm_TWa_2_MWh * 1.2;
...
d(h) = d_y_reg('2018',"DEU",h) * remind_totdemand("2030", "DEU", "seel") * sm_TWa_2_MWh / DIETER_OLDtotdem;
...
* Constraints on run of river (don't remove!)
con3i_maxprod_ror(h)..
        G_L('ror',h)
%reserves%$ontext
        + RP_CON('PR_up','ror',h)
...
$ontext
$offtext
        =L= phi_ror("2013","DEU",h)*N_CON('ror')
;
...
*set varname /Capacity, relativePrice, CapFac/;
*Parameters
*report 'report'
*con_MarketPrice(ct) 'market price for conventional supply tech'
*res_MarketPrice(res) 'market price for renewable supply tech'
*load_weighted_price
**capacityFactor(con)
**capacityFactor(res)
*;
...
**===========================================================
**============== reporting ==================================
*load_weighted_price =  -sum(h,con1a_bal.m(h)*d(h))/sum(h,d(h));
*
**=============== supply tech market price ==================
*con_MarketPrice(ct) = sum( h , G_L.l(ct,h)*(-con1a_bal.m(h)))/sum( h , G_L.l(ct,h));
**? include curtailment?
*res_MarketPrice(res) = sum( h , G_RES.l(res,h)*(-con1a_bal.m(h)))/sum( h , G_RES.l(res,h) + CU.l(res,h) );
*
**=============== demand tech market price ==================
*
*report("2030", "relativePrice", ct) = con_MarketPrice(ct) - load_weighted_price;
*report("2030", "relativePrice", res) = res_MarketPrice(res) - load_weighted_price;
*
*report("2030", "CapFac", ct)= sum( h , G_L.l(ct,h)) / N_CON(ct) * 8760;
*report("2030", "CapFac", res) = sum( h , G_RES.l(res,h)) /P_RES(res) * 8760;
*
*execute_unload "results_DIETER_1", report;
*
execute_unload "results_DT_all";
