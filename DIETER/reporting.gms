...
*       LCOE_marg $/MWh -> $/kWh
        report_tech('DIETER',yr,reg,'DIETER LCOE_marg ($/kWh)',ct)$(sum( h$(G_L.l(ct,h) = N_CON.l(ct) ) , G_L.l(ct,h)) ne 0) = ( ( c_i(ct) + cdata("c_fix_con",ct) ) *  N_CON.l(ct) / sum( h$(G_L.l(ct,h) = N_CON.l(ct) ) , G_L.l(ct,h))
                                                                                + con_fuelprice_reg(ct,reg) + cdata("carbon_content",ct)/cdata("eta_con",ct) * con_CO2price ) / 1e3;

        report_tech('DIETER',yr,reg,'DIETER LCOE_marg ($/kWh)',res)$(sum( h$(G_RES.l(res,h) = P_RES.l(res) ) , G_RES.l(res,h)) ne 0) = ( ( c_i_res(res) + rdata("c_fix_res",res) ) * P_RES.l(res) / sum( h$(G_RES.l(res,h) = P_RES.l(res) ) , G_RES.l(res,h)) ) / 1e3;
