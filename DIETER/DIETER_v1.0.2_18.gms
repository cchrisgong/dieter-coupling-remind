...
This version constitutes a minor revision of the model documented in Zerrahn, A., Schill, W.-P. (2150): A greenfield model to evaluate long-run power storage requirements for high shares of renewables. DIW Discussion Paper 1457. http://www.diw.de/documents/publikationen/73/diw_01.c.498475.de/dp1457.pdf
...
Sets
*============== remind sets ==================
yr          year for remind power sector             /2150/
yr_before   previous year from remind                /2130/
...
totFixedLoad = remind_totdemand("2150", "DEU", "seel") * sm_TWa_2_MWh;
*totFlexLoad = remind_totdemand("2150", "DEU", "seel") * sm_TWa_2_MWh * (1 - P);
...
earlyRetiCap_reporting(yr, reg, te_remind)$(remind_capEarlyReti(yr, reg, te_remind) ne 1) = (remind_capEarlyReti(yr, reg, te_remind) - remind_capEarlyReti2("2130", reg, te_remind) ) * remind_cap(yr, reg, te_remind, "1")
...
*P_RES.lo("Solar") = preInv_remind_prodSe("2150", "DEU", "pesol", "seel", "spv") * sm_TWa_2_MWh / capfac_const("Solar") ;
*P_RES.lo("Wind_on") = preInv_remind_prodSe("2150", "DEU", "pewin", "seel", "wind") * sm_TWa_2_MWh / capfac_const("Wind_on") ;
*N_CON.lo("ror") = preInv_remind_prodSe("2150", "DEU", "pehyd", "seel", "hydro") * sm_TWa_2_MWh / (capfac_ror * 8760) ;
*
*****************
...
*                    sum(   grade, preInv_remind_cap("2150", "DEU", te_remind, grade)$(COALte(te_remind))   )
...
*                   sum(   grade, preInv_remind_cap("2150", "DEU", te_remind, grade)$(NUCte(te_remind))   )
*                    ) * 1e6, 0.2* SMax(h, d(h)));
*
*N_CON.lo("CCGT") = min(sum(te_remind,
*                    sum(   grade, preInv_remind_cap("2150", "DEU", te_remind, grade)$(NonPeakGASte(te_remind))   )
*                    ) * 1e6, 0.2* SMax(h, d(h)));
*
*N_CON.lo("OCGT_eff") = min(sum(grade, preInv_remind_cap("2150", "DEU", "ngt", grade)) * 1e6, 0.2* SMax(h, d(h)));
*
*      
*N_CON.lo("bio") =  min(sum(te_remind,
*                    sum(   grade, preInv_remind_cap("2150", "DEU", te_remind, grade)$(BIOte(te_remind))   )
*                    ) * 1e6, 0.2* SMax(h, d(h)));

*
*** if no early retirement from last REMIND iteration, DIETER gets REMIND capacity as lower bound      
*N_CON.lo("lig")$(sum(te_remind, earlyRetiCap_reporting("2150", "DEU", te_remind)$(COALte(te_remind))) = 0) = sum(te_remind,
*                    sum(   grade, preInv_remind_cap("2150", "DEU", te_remind, grade)$(COALte(te_remind))   )
*                    ) * 1e6 /2;
*
*N_CON.lo("hc")$(sum(te_remind, earlyRetiCap_reporting("2150", "DEU", te_remind)$(COALte(te_remind))) = 0) = sum(te_remind,
*                    sum(   grade, preInv_remind_cap("2150", "DEU", te_remind, grade)$(COALte(te_remind))   )
*                    ) * 1e6 /2;
*                    
*N_CON.lo("nuc")$(sum(te_remind, earlyRetiCap_reporting("2150", "DEU", te_remind)$(NUCte(te_remind))) = 0)  = sum(te_remind,
*                   sum(   grade, preInv_remind_cap("2150", "DEU", te_remind, grade)$(NUCte(te_remind))   )
*                  ) * 1e6;
*
*N_CON.lo("CCGT")$(sum(te_remind, earlyRetiCap_reporting("2150", "DEU", te_remind)$(NonPeakGASte(te_remind))) = 0) = sum(te_remind,
*                    sum(   grade, preInv_remind_cap("2150", "DEU", te_remind, grade)$(NonPeakGASte(te_remind))   )
*                    ) * 1e6;
*
*N_CON.lo("OCGT_eff")$(earlyRetiCap_reporting("2150", "DEU", "ngt") = 0) = sum(grade, preInv_remind_cap("2150", "DEU", "ngt", grade)) * 1e6;
*
*      
*N_CON.lo("bio")$(sum(te_remind, earlyRetiCap_reporting("2150", "DEU", te_remind)$(BIOte(te_remind))) = 0) =  sum(te_remind,
*                    sum(   grade, preInv_remind_cap("2150", "DEU", te_remind, grade)$(BIOte(te_remind))   )
*                    ) * 1e6;
*
**** if there is early retirement from last REMIND iteration, DIETER gets REMIND capacity as fx
*N_CON.fx("lig")$(sum(te_remind, earlyRetiCap_reporting("2150", "DEU", te_remind)$(COALte(te_remind))) > 0) =
*                        RM_postInv_cap_con("2150", "DEU", "coal") * 1e6 /2;
*
*N_CON.fx("hc")$(sum(te_remind, earlyRetiCap_reporting("2150", "DEU", te_remind)$(COALte(te_remind))) > 0) =
*                        RM_postInv_cap_con("2150", "DEU", "coal") * 1e6 /2;
*                    
*N_CON.fx("nuc")$(sum(te_remind, earlyRetiCap_reporting("2150", "DEU", te_remind)$(NUCte(te_remind))) > 0)  =
*                        RM_postInv_cap_con("2150", "DEU", "nuc") * 1e6;
*
*N_CON.fx("CCGT")$(sum(te_remind, earlyRetiCap_reporting("2150", "DEU", te_remind)$(NonPeakGASte(te_remind))) > 0) =
*                        RM_postInv_cap_con("2150", "DEU", "CCGT") * 1e6;
*
*N_CON.fx("OCGT_eff")$(earlyRetiCap_reporting("2150", "DEU", "ngt") > 0) = RM_postInv_cap_con("2150", "DEU", "OCGT_eff") * 1e6;
*
*
*N_CON.fx("bio")$(sum(te_remind, earlyRetiCap_reporting("2150", "DEU", te_remind)$(BIOte(te_remind))) > 0) =
*                        RM_postInv_cap_con("2150", "DEU", "bio") * 1e6;
...
*P_RES.fx("Solar") = remind_prodSe("2150", "DEU", "pesol", "seel", "spv") * sm_TWa_2_MWh / capfac_const("Solar") ;
*P_RES.fx("Wind_on") = remind_prodSe("2150", "DEU", "pewin", "seel", "wind") * sm_TWa_2_MWh / capfac_const("Wind_on") ;
*N_CON.fx("ror") = remind_prodSe("2150", "DEU", "pehyd", "seel", "hydro") * sm_TWa_2_MWh / (capfac_ror * 8760) ;
*N_CON.fx("CCGT")= RM_postInv_cap_con("2150", "DEU", "CCGT") ;
*N_CON.fx("OCGT_eff")= RM_postInv_cap_con("2150", "DEU", "OCGT_eff") ;
*N_CON.fx("bio")= RM_postInv_cap_con("2150", "DEU", "bio") ;
*N_CON.fx("nuc")= RM_postInv_cap_con("2150", "DEU", "nuc") ;
*N_CON.fx("lig") = RM_postInv_cap_con("2150", "DEU", "coal")/2 ;
*N_CON.fx("hc") = RM_postInv_cap_con("2150", "DEU", "coal")/2 ;
...
*N_STO_P.fx('Sto1') = remind_cap("2150", "DEU", "storspv", "1") * 3 * 1e6 + remind_cap("2150", "DEU", "storwind", "1") * 0.3* 1e6;
...
*1.2 is the conversion btw 2130$ and 2015$
...
con_fuelprice_reg0("lig",reg) = -remind_fuelcost2("2130",reg,"pecoal") / (-remind_budget2("2130",reg)) * 1e12 / sm_TWa_2_MWh * 1.2 - 3.6;
con_fuelprice_reg0("hc",reg) = -remind_fuelcost2("2130",reg,"pecoal") / (-remind_budget2("2130",reg)) * 1e12 / sm_TWa_2_MWh * 1.2 + 1.8;
con_fuelprice_reg0("CCGT",reg) = -remind_fuelcost2("2130",reg,"pegas") / (-remind_budget2("2130",reg)) * 1e12 / sm_TWa_2_MWh * 1.2;
con_fuelprice_reg0("bio",reg) = -remind_fuelcost2("2130",reg,"pebiolc") / (-remind_budget2("2130",reg)) * 1e12 / sm_TWa_2_MWh * 1.2;
con_fuelprice_reg0("nuc",reg) = -remind_fuelcost2("2130",reg,"peur") / (-remind_budget2("2130",reg)) * 1e12 / sm_TWa_2_MWh * 1.2;

con_fuelprice_reg1("lig",reg) = -remind_fuelcost("2150",reg,"pecoal") / (-remind_budget("2150",reg)) * 1e12 / sm_TWa_2_MWh * 1.2 - 3.6;
con_fuelprice_reg1("hc",reg) = -remind_fuelcost("2150",reg,"pecoal") / (-remind_budget("2150",reg)) * 1e12 / sm_TWa_2_MWh * 1.2 + 1.8;
con_fuelprice_reg1("CCGT",reg) = -remind_fuelcost("2150",reg,"pegas") / (-remind_budget("2150",reg)) * 1e12 / sm_TWa_2_MWh * 1.2;
con_fuelprice_reg1("bio",reg) = -remind_fuelcost("2150",reg,"pebiolc") / (-remind_budget("2150",reg)) * 1e12 / sm_TWa_2_MWh * 1.2;
con_fuelprice_reg1("nuc",reg) = -remind_fuelcost("2150",reg,"peur") / (-remind_budget("2150",reg)) * 1e12 / sm_TWa_2_MWh * 1.2;
...
cdata("eta_con","lig") = remind_eta1("2150","DEU","pc");
cdata("eta_con","hc") = remind_eta1("2150","DEU","pc") * 0.92;
cdata("eta_con","CCGT") = remind_eta1("2150","DEU","ngcc");
cdata("eta_con","OCGT_eff") = remind_eta1("2150","DEU","ngt");
cdata("eta_con","bio") = remind_eta1("2150","DEU","bioigcc");
cdata("eta_con","ror") = remind_eta2("2150","DEU","hydro");
cdata("eta_con","nuc") = remind_eta2("2150","DEU","tnrs");
...
c_i_ovnt("lig") = remind_CapCost("2150", "DEU", "pc") * 1e6 * 1.2 ;
c_i_ovnt("hc") = c_i_ovnt("lig") + 300000 ;
c_i_ovnt("CCGT") = remind_CapCost("2150", "DEU", "ngcc")  * 1e6 * 1.2;
c_i_ovnt("OCGT_eff") = remind_CapCost("2150", "DEU", "ngt") * 1e6 * 1.2;
c_i_ovnt("bio") = remind_CapCost("2150", "DEU", "biochp") * 1e6 * 1.2;
c_i_ovnt("ror") = remind_CapCost("2150", "DEU", "hydro") * 1e6 * 1.2;
c_i_ovnt("nuc") = remind_CapCost("2150", "DEU", "tnrs") * 1e6 * 1.2;
 
c_i_ovnt_res("Solar") = remind_CapCost("2150", "DEU", "spv") * 1e6 * 1.2 ;
c_i_ovnt_res("Wind_on") = remind_CapCost("2150", "DEU", "wind") * 1e6 * 1.2;
...
execute_unload "results_DIETER_y18", report4RM, reportmk_4RM;

execute_unload "full_DIETER_y18";
execute_unload "report_DIETER_y18", report, report_tech, report_hours, report_tech_hours;
