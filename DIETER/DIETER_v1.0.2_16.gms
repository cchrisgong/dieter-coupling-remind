...
This version constitutes a minor revision of the model documented in Zerrahn, A., Schill, W.-P. (2110): A greenfield model to evaluate long-run power storage requirements for high shares of renewables. DIW Discussion Paper 1457. http://www.diw.de/documents/publikationen/73/diw_01.c.498475.de/dp1457.pdf
...
Sets
*============== remind sets ==================
yr          year for remind power sector             /2110/
yr_before   previous year from remind                /2100/
...
earlyRetiCap_reporting(yr, reg, te_remind)$(remind_capEarlyReti(yr, reg, te_remind) ne 1) = (remind_capEarlyReti(yr, reg, te_remind) - remind_capEarlyReti2("2100", reg, te_remind) ) * remind_cap(yr, reg, te_remind, "1")
...
P_RES.lo("Solar") = preInv_remind_prodSe("2110", "DEU", "pesol", "seel", "spv") * sm_TWa_2_MWh / capfac_const("Solar") ;
P_RES.lo("Wind_on") = preInv_remind_prodSe("2110", "DEU", "pewin", "seel", "wind") * sm_TWa_2_MWh / capfac_const("Wind_on") ;
N_CON.lo("ror") = preInv_remind_prodSe("2110", "DEU", "pehyd", "seel", "hydro") * sm_TWa_2_MWh / (capfac_ror * 8760) ;
...
N_CON.lo("lig") = sum(te_remind,
                    sum(   grade, preInv_remind_cap("2110", "DEU", te_remind, grade)$(COALte(te_remind))   )
                    ) * 1e6 /2;
...
N_CON.lo("nuc") = sum(te_remind,
                   sum(   grade, preInv_remind_cap("2110", "DEU", te_remind, grade)$(NUCte(te_remind))   )
                  ) * 1e6;

N_CON.lo("CCGT") = sum(te_remind,
                    sum(   grade, preInv_remind_cap("2110", "DEU", te_remind, grade)$(NonPeakGASte(te_remind))   )
                    ) * 1e6;

N_CON.lo("OCGT_eff") = sum(grade, preInv_remind_cap("2110", "DEU", "ngt", grade)) * 1e6;

      
N_CON.lo("bio") =  sum(te_remind,
                    sum(   grade, preInv_remind_cap("2110", "DEU", te_remind, grade)$(BIOte(te_remind))   )
                    ) * 1e6;
                    
*
*** if no early retirement from last REMIND iteration, DIETER gets REMIND capacity as lower bound      
*N_CON.lo("lig")$(sum(te_remind, earlyRetiCap_reporting("2110", "DEU", te_remind)$(COALte(te_remind))) = 0) = sum(te_remind,
*                    sum(   grade, preInv_remind_cap("2110", "DEU", te_remind, grade)$(COALte(te_remind))   )
*                    ) * 1e6 /2;
*
*N_CON.lo("hc")$(sum(te_remind, earlyRetiCap_reporting("2110", "DEU", te_remind)$(COALte(te_remind))) = 0) = sum(te_remind,
*                    sum(   grade, preInv_remind_cap("2110", "DEU", te_remind, grade)$(COALte(te_remind))   )
*                    ) * 1e6 /2;
*                    
*N_CON.lo("nuc")$(sum(te_remind, earlyRetiCap_reporting("2110", "DEU", te_remind)$(NUCte(te_remind))) = 0)  = sum(te_remind,
*                   sum(   grade, preInv_remind_cap("2110", "DEU", te_remind, grade)$(NUCte(te_remind))   )
*                  ) * 1e6;
*
*N_CON.lo("CCGT")$(sum(te_remind, earlyRetiCap_reporting("2110", "DEU", te_remind)$(NonPeakGASte(te_remind))) = 0) = sum(te_remind,
*                    sum(   grade, preInv_remind_cap("2110", "DEU", te_remind, grade)$(NonPeakGASte(te_remind))   )
*                    ) * 1e6;
*
*N_CON.lo("OCGT_eff")$(earlyRetiCap_reporting("2110", "DEU", "ngt") = 0) = sum(grade, preInv_remind_cap("2110", "DEU", "ngt", grade)) * 1e6;
*
*      
*N_CON.lo("bio")$(sum(te_remind, earlyRetiCap_reporting("2110", "DEU", te_remind)$(BIOte(te_remind))) = 0) =  sum(te_remind,
*                    sum(   grade, preInv_remind_cap("2110", "DEU", te_remind, grade)$(BIOte(te_remind))   )
*                    ) * 1e6;
*
**** if there is early retirement from last REMIND iteration, DIETER gets REMIND capacity as fx
*N_CON.fx("lig")$(sum(te_remind, earlyRetiCap_reporting("2110", "DEU", te_remind)$(COALte(te_remind))) > 0) =
*                        RM_postInv_cap_con("2110", "DEU", "coal") * 1e6 /2;
*
*N_CON.fx("hc")$(sum(te_remind, earlyRetiCap_reporting("2110", "DEU", te_remind)$(COALte(te_remind))) > 0) =
*                        RM_postInv_cap_con("2110", "DEU", "coal") * 1e6 /2;
*                    
*N_CON.fx("nuc")$(sum(te_remind, earlyRetiCap_reporting("2110", "DEU", te_remind)$(NUCte(te_remind))) > 0)  =
*                        RM_postInv_cap_con("2110", "DEU", "nuc") * 1e6;
*
*N_CON.fx("CCGT")$(sum(te_remind, earlyRetiCap_reporting("2110", "DEU", te_remind)$(NonPeakGASte(te_remind))) > 0) =
*                        RM_postInv_cap_con("2110", "DEU", "CCGT") * 1e6;
*
*N_CON.fx("OCGT_eff")$(earlyRetiCap_reporting("2110", "DEU", "ngt") > 0) = RM_postInv_cap_con("2110", "DEU", "OCGT_eff") * 1e6;
*
*
*N_CON.fx("bio")$(sum(te_remind, earlyRetiCap_reporting("2110", "DEU", te_remind)$(BIOte(te_remind))) > 0) =
*                        RM_postInv_cap_con("2110", "DEU", "bio") * 1e6;
...
*P_RES.fx("Solar") = remind_prodSe("2110", "DEU", "pesol", "seel", "spv") * sm_TWa_2_MWh / capfac_const("Solar") ;
*P_RES.fx("Wind_on") = remind_prodSe("2110", "DEU", "pewin", "seel", "wind") * sm_TWa_2_MWh / capfac_const("Wind_on") ;
*N_CON.fx("ror") = remind_prodSe("2110", "DEU", "pehyd", "seel", "hydro") * sm_TWa_2_MWh / (capfac_ror * 8760) ;
*N_CON.fx("CCGT")= RM_postInv_cap_con("2110", "DEU", "CCGT") ;
*N_CON.fx("OCGT_eff")= RM_postInv_cap_con("2110", "DEU", "OCGT_eff") ;
*N_CON.fx("bio")= RM_postInv_cap_con("2110", "DEU", "bio") ;
*N_CON.fx("nuc")= RM_postInv_cap_con("2110", "DEU", "nuc") ;
*N_CON.fx("lig") = RM_postInv_cap_con("2110", "DEU", "coal")/2 ;
*N_CON.fx("hc") = RM_postInv_cap_con("2110", "DEU", "coal")/2 ;
...
*N_STO_P.fx('Sto1') = remind_cap("2110", "DEU", "storspv", "1") * 3 * 1e6+ remind_cap("2110", "DEU", "storwind", "1") * 0.3* 1e6;
...
*1.2 is the conversion btw 2100$ and 2015$
...
con_fuelprice_reg("lig",reg) = -remind_fuelcost("2110",reg,"pecoal") / (-remind_budget("2110",reg)) * 1e12 / sm_TWa_2_MWh * 1.2 - 3.6;
con_fuelprice_reg("hc",reg) = -remind_fuelcost("2110",reg,"pecoal") / (-remind_budget("2110",reg)) * 1e12 / sm_TWa_2_MWh * 1.2 + 1.8;
con_fuelprice_reg("CCGT",reg) = -remind_fuelcost("2110",reg,"pegas") / (-remind_budget("2110",reg)) * 1e12 / sm_TWa_2_MWh * 1.2;
con_fuelprice_reg("OCGT_eff",reg) = con_fuelprice_reg("CCGT",reg);
con_fuelprice_reg("bio",reg) = -remind_fuelcost("2110",reg,"pebiolc") / (-remind_budget("2110",reg)) * 1e12 / sm_TWa_2_MWh * 1.2;
con_fuelprice_reg("nuc",reg) = -remind_fuelcost("2110",reg,"peur") / (-remind_budget("2110",reg)) * 1e12 / sm_TWa_2_MWh * 1.2;
con_fuelprice_reg("ror",reg) = 0;

*eta from remind
** split pecoal into lignite and hc for rough comparison (not finalized)
cdata("eta_con","lig") = remind_eta1("2110","DEU","pc");
cdata("eta_con","hc") = remind_eta1("2110","DEU","pc") * 0.92;
cdata("eta_con","CCGT") = remind_eta1("2110","DEU","ngcc");
cdata("eta_con","OCGT_eff") = remind_eta1("2110","DEU","ngt");
cdata("eta_con","bio") = remind_eta1("2110","DEU","bioigcc");
cdata("eta_con","ror") = remind_eta2("2110","DEU","hydro");
cdata("eta_con","nuc") = remind_eta2("2110","DEU","tnrs");
...
c_i_ovnt("lig") = remind_CapCost("2110", "DEU", "pc") * 1e6 * 1.2 ;
c_i_ovnt("hc") = c_i_ovnt("lig") + 300000 ;
c_i_ovnt("CCGT") = remind_CapCost("2110", "DEU", "ngcc")  * 1e6 * 1.2;
c_i_ovnt("OCGT_eff") = remind_CapCost("2110", "DEU", "ngt") * 1e6 * 1.2;
c_i_ovnt("bio") = remind_CapCost("2110", "DEU", "biochp") * 1e6 * 1.2;
c_i_ovnt("ror") = remind_CapCost("2110", "DEU", "hydro") * 1e6 * 1.2;
c_i_ovnt("nuc") = remind_CapCost("2110", "DEU", "tnrs") * 1e6 * 1.2;
 
c_i_ovnt_res("Solar") = remind_CapCost("2110", "DEU", "spv") * 1e6 * 1.2 ;
c_i_ovnt_res("Wind_on") = remind_CapCost("2110", "DEU", "wind") * 1e6 * 1.2;
...
totFixedLoad = remind_totdemand("2110", "DEU", "seel") * sm_TWa_2_MWh;
*totFlexLoad = remind_totdemand("2110", "DEU", "seel") * sm_TWa_2_MWh * (1 - P);
...
execute_unload "results_DIETER_y16", report4RM, reportmk_4RM;

execute_unload "full_DIETER_y16";
execute_unload "report_DIETER_y16", report, report_tech, report_hours, report_tech_hours;
